...
// ==========================================
// PRIZE MANAGEMENT
// Assign / view / update a prize for an already-selected daily or weekly winner.
// Winner selection logic above is untouched — these only attach a Prize to
// the existing Winner document via its (already-modeled) prizeId field.
// ==========================================

export const assignDailyPrize = async (req, res) => {
    try {
        ...
        if (!winner) {
            return res.status(404).json({
                success: false,
                message: 'No daily winner found for today. Please select a winner first.'
            });
        }
        ...
        if (winner.prizeId) {
            return res.status(400).json({
                success: false,
                message: "A prize has already been assigned to today's winner. Use update instead."
            });
        }
        ...
        if (req.file) {
            ...
            if (!uploaded) {
                return res.status(500).json({
                    success: false,
                    message: 'Prize image upload failed'
                });
            }
        ...
        try {
            await NotificationService.sendPrizeAssignedNotification(
                winner.userId,
                prize._id,
                prize.title,
                'daily'
            ...
            console.error('Error sending prize assigned notification:', notificationError);
        }

        res.status(201).json({
            success: true,
            message: 'Prize assigned to daily winner successfully',
            data: { prize }
        });
    } catch (error) {
        console.error('Error assigning daily prize:', error);
        res.status(500).json({
            success: false,
            message: 'Internal server error',
            error: error.message
...
export const getDailyPrize = async (req, res) => {
    try {
        const start = new Date();
        start.setHours(0, 0, 0, 0);
        const end = new Date();
...
export const assignWeeklyPrize = async (req, res) => {
    try {
        const { title, description } = req.body;
        ...
        const winner = await Winner.findOne({
            cycleType: 'weekly',
            winnerDate: { $gte: startOfWeek, $lte: endOfWeek }
        });

        if (!winner) {
            return res.status(404).json({
                success: false,
                message: 'No weekly winner found for this week. Please select a winner first.'
            });
        }
        ...
        if (winner.prizeId) {
            return res.status(400).json({
                success: false,
                message: "A prize has already been assigned to this week's winner. Use update instead."
            });
        }
        ...
        res.status(201).json({
            success: true,
            message: 'Prize assigned to weekly winner successfully',
            data: { prize }
        });
    } catch (error) {
        console.error('Error assigning weekly prize:', error);
        res.status(500).json({
            success: false,
            message: 'Internal server error',
            error: error.message
...
export const getWeeklyPrize = async (req, res) => {
    try {
        const today = new Date();
        const startOfWeek = new Date(today);
        startOfWeek.setDate(today.getDate() - today.getDay() + 1);
        ...
        console.error('Error fetching weekly prize:', error);
        res.status(500).json({
            success: false,
            message: 'Internal server error'
        });
